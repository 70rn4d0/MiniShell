		...
		if (ft_strcmp(env_lst->data, expand_word) == 0)
		{
			if (env_lst->value)
			{
				expanded_word = (char *)malloc(sizeof(char)
						* ft_strlen(env_lst->value->data) + 1);
				alloc_addback(&g_allocs, expanded_word);
				expanded_word[ft_strlen(env_lst->value->data)] = '\0';
				expanded_word = ft_strncpy(expanded_word, env_lst->value->data,
						ft_strlen(env_lst->value->data));
				return (expanded_word);
			}
			else
			{
